...
export const logoutUser = CatchAsyncError(async(req:Request , res:Response , next:NextFunction) => {
    try {
         // Xóa cookie với đầy đủ cấu hình phù hợp
         res.cookie("access_token", "", {
            maxAge: 1,
            httpOnly: true,
            secure: true, // Bắt buộc trên HTTPS
            sameSite: "strict", // Hoặc "lax" tùy trường hợp
            path: "/", // Đảm bảo đường dẫn giống như khi cookie được tạo
        });
        ...
        res.cookie("refresh_token", "", {
            ...
            path: "/", // Đường dẫn phải giống như khi cookie được tạo
        });

        // Kiểm tra và xóa token liên quan đến user trong Redis (nếu có)
        const userId = req.user?._id || "";
        if (userId) {
            await redis.del(userId); // Sử dụng Redis để xóa token
        }
        res.status(200).json({
            success:true,
